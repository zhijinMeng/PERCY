...
class Predictor(inference.Inference):
    def __init__(self, cfg):
        ...
        self.listemo = ["angry", "disgust", "fear", "happy", "neutral", "sad", "surprise"]
    ...
    def predict(self, faces):
        """Classify the detected faces"""
        if len(faces) > 0:
            # Move all input tensors to CPU
            faces = [torch.tensor(face).to(self.device) for face in faces]
            pred_index, pred_score = self.inference(faces)
        else:
            pred_index, pred_score = np.zeros((0,)), np.zeros((0,))
        # print(self.listemo[pred_index[0]])
