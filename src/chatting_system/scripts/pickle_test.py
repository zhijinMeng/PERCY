...
class VoiceVerification:
    def __init__(self):
        self.pickle_files = {}
        self.MODEL_PATH = '/home/robocupathome/workspace/eddy_code/src/chatting_system/scripts/voice_verification_model.h5'
    ...
    def enroll_new_user(self, id, enroll_wav_path, pkl_path):
        # file_path = f'{id}.pkl'
        mfcc = sample_from_mfcc(read_mfcc(enroll_wav_path, SAMPLE_RATE), NUM_FRAMES)
        with open(pkl_path, 'wb') as f:
            pickle.dump(mfcc, f)
        self.pickle_files[id] = pkl_path
    ...
    def verify_user(self, id, verify_wav_path):
        ...
        if loaded_mfcc is not None:
            ...
            score = self.give_score(prediction, verify_prediction)
            return score
        else:
            return None
    ...
    def give_score(self,prediction,verify_prediction):
